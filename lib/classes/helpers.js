...
function TweetHelperClass () {
  ...
  this.getInviteTweet = function () {
    return {
      userId: null,
      text: 'Repeeet tweet easily with '+Meteor.absoluteUrl()+' “Trust me and try repeating your tweets” - Guy Kawasaki',
      date: new Date().getTime(),
  ...
  this.formatTweetText = function (text) {
    var regexLink = /(https?:\/\/?[^ ]+)/gi;
    var regexHashtag = /#([^ ]+)/gi;
    var regexMention = /@([^ ]+)/gi;
    var htmlLink = '<a href="$1">$1</a>';
    var htmlHashtag = '<a href="https://twitter.com/search?q=%23$1">#$1</a>';
    var htmlMention = '<a href="http://twitter.com/$1">@$1</a>';
    return text
      .replace(regexLink, htmlLink)
      .replace(regexHashtag, htmlHashtag)
      .replace(regexMention, htmlMention);
  }
}
